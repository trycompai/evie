...
| Approval card options | A joined bordered list with a leading letter square per option | shadcn `questionnaire` choice cards: radio indicator, trailing shortcut chip | Every AI-asked questionnaire goes through the shadcn questionnaire component, which brings fieldset/radio semantics and letter shortcuts that actually work. The file's letter square was display-only — a shortcut the keyboard never honoured. |
| Chat surfaces | Bespoke bubble/divider markup | Built on the shadcn chat components (`bubble`, `marker`, `attachment`, `message`; `message-scroller` is vendored, see `apps/web/src/components/timeline.tsx` for why the thread does not use it) | One vocabulary for chat UI. Evie's shapes — the 780px cap, `rounded-bubble`, Paper's raised surfaces — ride on top as classNames, so the drawing is unchanged where it mattered. |
| Thread scrolling | `MessageScroller`'s scroll control | The same control, reimplemented in `apps/web/src/components/timeline.tsx` against its virtualizer | The component is the wrong trade for a 2,000-row thread; a way back to the live edge after scrolling away is not. |
| Anchored turns / last-anchor open | Turn anchored to the top of the viewport, via a bottom spacer | Omitted | Built, then removed. Anchoring only earns its keep when a reply is longer than the viewport, and by then the spacer has shrunk to nothing — so all it does in practice is hold a screenful of dead space above the composer after a short reply. |
| Thread viewport `scroll-fade` | Edge fade on the scroll container | Omitted | A mask on the one surface that scrolls two thousand rows re-rasterizes the visible tile per scroll tick. Kept on `AttachmentGroup`, where the strip is five items. |
