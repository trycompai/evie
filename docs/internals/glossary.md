...
| **routine** | A cron row that dispatches a turn. Carries its own IANA timezone, never the host's. **Paused** (`enabled = 0`) is a person's choice and reverses with the same control; **blocked** (`blocked_reason`) is the scheduler withdrawing the routine itself, and resuming will not fix it. `next_run_at` is a cache the scheduler recomputes only for rows it might fire, so `routines.list` reports it as null for both — a paused routine keeps a real due time that drifts into the past, and rendering it would be a countdown that lies. | [`src/scheduler/`](../../apps/server/src/scheduler) |
| **connection** | An authored file under a bot's `agent/connections/`, scoped `org` or `member`. Evie **generates** it from the bot's rows on every runtime spawn and sweeps the ones it wrote for connections that are gone — a disconnected service whose file survived would keep working, which is the worst direction for that bug to fail in. `org` scope reads its token from `connectionEnvName(name)`; `member` scope is not generated, because per-person OAuth still needs the callback proxy 05 describes. | [`src/provider/scaffold.ts`](../../apps/server/src/provider/scaffold.ts) |
| **grant** | One member's authorization of a `member`-scoped connection. | [`contracts/commands.ts`](../../packages/contracts/src/commands.ts) |
| **capability briefing** | The Evie-owned system instruction at a bot's `agent/instructions/00-evie.md`, describing the runtime it actually has: its tools, its sandbox, its connections, and the fact that **routines**, not eve's `agent/schedules/`, are how it runs on a schedule. Rewritten on every runtime spawn, so a correction reaches bots scaffolded months ago. Separate from the user's `instructions.md` precisely so it can be corrected without eating their persona. | [`src/provider/scaffold.ts`](../../apps/server/src/provider/scaffold.ts) |
...
| **provisioning** | Writing a new bot's eve project to disk, installing it, and booting its runtime. `BotProvisioned` lands only after eve answered its health route — the receipt means "can take a message", not "files on disk" — and the turn reactor answers it with a greeting turn, so the bot speaks first. Slow — `git init` plus `npm install` plus an eve boot, because eve is pinned per bot — so it belongs to the supervisor reactor, behind the bot's health chip. It is **not** part of the projection: it lived there once, and the bot row did not exist until the install finished. | [`src/reactors/supervisor.ts`](../../apps/server/src/reactors/supervisor.ts) |
...
| **presence** | Which threads a client has open and which reasoning blocks it expanded. Drives subscriptions and idle-stop. Server-side, `ClientPresence` answers "is anyone watching this bot?" from the hub's **live subscriber set**, not from `presence.set` — a subscription withdraws itself when its scope unwinds, so a dropped socket cannot leave a runtime warm forever. | [`client-runtime/presence.ts`](../../packages/client-runtime/src/presence.ts), [`src/reactors/supervisor.ts`](../../apps/server/src/reactors/supervisor.ts) |
